...
The raw day-by-day data files can be found online at [Prof. Nicholas Horton's website](https://nhorton.people.amherst.edu/valleybikes/), in the form of .csv and .csv.gz files (i.e. compressed .csv format). For full offline functionality of the package, these files are also stored in the [inst/extdata directory](https://github.com/Amherst-Statistics/valleybikeData/tree/master/inst/extdata) of the [valleybikeData package](https://github.com/Amherst-Statistics/valleybikeData), although note that .csv files are compressed to .csv.gz during the download process in order to reduce their size and standardize the extension used.

At the time of writing, there are 507 daily data files in total, covering all active ValleyBike days from 28 June 2018 to 5 October 2020. 

## Data Download

When new files are added to the website, they need to be synced with the package (i.e. downloaded to inst/extdata). This can only be done by package authors, contributors, or anyone with write access to the valleybikeData repository. To sync the files appropriately, the `download_data` utility function is provided. Please do not perform this process by hand. The `download_data` function can be used as follows:
...
3. Run `download_data`. It takes one required parameter, `path`, which specifies the path where the files will be downloaded. This should be your own local path to the cloned repository's inst/extdata directory, e.g. "~/git/valleybikeData/inst/extdata". The function also takes an optional parameter, `overwrite`, which specifies whether to overwrite the already-existing files in inst/extdata (defaults to `FALSE` for efficiency). You would only need to set `overwrite = TRUE` if the old files have been changed and you want to replace them.
...
6 files currently contain corrupted data, and the process of fixing them is under way. The corrupted files all measure 161 bytes and correspond to the following days:
...
They are kept in the inst/extdata folder for the sake of thoroughness. All utility functions that deal with .csv.gz files can recognize and filter out these files.
...
To import a month's worth of data from the raw .csv.gz daily files, the `import_month` utility function is provided. It uses `import_day` repeatedly behind the scenes in order to collate the data from all days of the desired month. 
...
To import all of the raw .csv.gz daily files into a single unified data frame (of 60+ million observations over 2018-2020), the `import_full` utility function is provided. Note that it takes quite long to run, and yields a rather large object.
